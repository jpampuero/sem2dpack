...
!!  use mat_user  
 ...
 ! working arrays for each element
  type matwrk_elem_type
    ...
    type(matwrk_elasM_type), pointer :: elasM=>null() !!**
    type(matwrk_dyndmg_type), pointer :: dydmg=>null() !!**
...
subroutine MAT_read(mat,iin,ndof) !!**
    ...
    if (echo_input) then
      ...
      do k=1,2
        select case(kind(k))
          ...
          case ('ELASM'); name(k) = 'Elastic-mt' !!**
          case ('DyDMG'); name(k) = 'Dynamic-Damage' !!**
   ...
   !NOTE: there is no rewind in these subroutines
    do k=1,2
      select case(kind(k))
        ...
        case ('ELASM'); call MAT_ELASM_read(mat(tag),iin,ndof) !!**
        case ('DyDMG'); call MAT_DyDMG_read(mat(tag),iin,ndof) !!**
...
subroutine MAT_init_prop(mat_elem,mat_input,grid)
 ...
 ! report memory allocations
  ...
  call storearray('matpro:elasM',MAT_ELASM_mempro,idouble) !!**
  call storearray('matpro:DyDMG',MAT_DyDMG_mempro,idouble) !!**
...
subroutine MAT_init_elem_prop(elem,ecoord)
  ...
  if (MAT_isElasticM(elem)) call MAT_ELASM_init_elem_prop(elem,ecoord) !!**
  if (MAT_isDynDamage(elem)) call MAT_DyDMG_init_elem_prop(elem,ecoord) !!**
...
subroutine MAT_init_work(matwrk,matpro,grid,ndof,dt)
 ...
 ! get the list of the first element of each material
  ...
  do e=1,grid%nelem
   ...
   ! so we put it on a separate 'if'
    ...
    elseif (MAT_isElasticM(matpro(e))) then !!**
      allocate(matwrk(e)%derint) !!**
      allocate(matwrk(e)%elasM) !!**
      call MAT_set_derint(matwrk(e)%derint,grid,e) !!**
      call MAT_ELASM_init_elem_work(matwrk(e)%elasM,matpro(e),grid%ngll,ndof) !!**
  
    elseif (MAT_isDynDamage(matpro(e))) then !!**
      allocate(matwrk(e)%derint) !!**
      allocate(matwrk(e)%DyDMG) !!**
      call MAT_set_derint(matwrk(e)%derint,grid,e) !!**
      call MAT_DyDMG_init_elem_work(matwrk(e)%DyDMG,matpro(e),grid%ngll,ndof) !!**
  
  !!  elseif (MAT_isUser(matpro(e)))
  !!	allocate(matwrk(e)%derint)
  !!    allocate(matwrk(e)%user)
  !!    call MAT_USER_init_elem_work(matwrk(e)%user,matpro,grid, ... )
 ...
 ! report memory allocations
  ...
  call storearray('matwrk%elasM',MAT_ELASM_memwrk,idouble) !!**
  call storearray('matwrk%DyDMG',MAT_DyDMG_memwrk,idouble) !!**
...
subroutine MAT_Fint(f,d,v,matpro,matwrk,ngll,ndof,dt,grid, E_ep,E_el,sg,sgp,tt)
...
!!**     call MAT_stress(s,e,matwrk,matpro,ngll,ndof,.true.,dt,E_ep,E_el,sg,sgp)
    call MAT_stress(s,e,matwrk,matpro,ngll,ndof,.true.,dt,E_ep,E_el,sg,sgp,tt)
...
subroutine MAT_stress(s,e,matwrk,matpro,ngll,ndof,update,dt,E_ep,E_el,sg,sgp,tt)
  ...
  if (MAT_isElasticM(matpro)) call MAT_ELASM_stress(s,e,matwrk%elasM,ngll,ndof,update,dt) !!**
  if (MAT_isDynDamage(matpro)) call MAT_DyDMG_stress(s,e,matwrk%DyDMG,ngll,ndof,update,dt,tt) !!**
...
subroutine MAT_write(matwrk,matpro,tag,ndof)
!!** subroutine MAT_write(matwrk,matpro,tag)
  ...
  call MAT_ELASM_write(matwrk,matpro,tag,ndof) !!** 
  call MAT_DyDMG_write(matwrk,matpro,tag,ndof) !!** 
...
!-----------------------------------------------------------------------
!!** (following subroutine is all new)
!!**  Export snapshots of state variables to binary file elm_xxx_sem2d.dat
!!**  (where xxx is the snapshot index):
!!**    
!!**  The ASCII file elm_elems_sem2d.tab contains the list of elements concerned,
!!**  in two columns:
!!**    col 1 = element index in the elasm_xxx_sem2d.dat file
!!**    col 2 = element index in the global mesh
!!** 
  subroutine MAT_ELASM_write(mw,mp,tag,ndof)
 ...
 ! initialize
  if (.not.initialized) then 
    iout = IO_new_unit()
    open(unit=iout,file='elm_elems_sem2d.tab')
    k=0
    do e = 1,nelem
      if ( .not. MAT_isElasticM(mp(e)) ) cycle
      k=k+1
      write(iout,*) k,e
      if (k==1) iol = e
    enddo
    close(iout)
    inquire( IOLENGTH=iol )  MAT_ELASM_export(mw(iol)%elasM,ndof)
    !MAT_ELASM_export(m,ndof) result(dat)
    initialized = .true.
  endif

 ! export snapshots
  write(fname,'("elm_",A,"_sem2d.dat")') trim(tag)
  iout = IO_new_unit()
  open(unit=iout,file=trim(fname),status='replace',access='direct',recl=iol)
  k=0
  do e = 1,nelem
    if ( .not. MAT_isElasticM(mp(e)) ) cycle
    k=k+1
    write(iout,rec=k) MAT_ELASM_export(mw(e)%elasM,ndof)
  enddo
  close(iout)

  end subroutine MAT_ELASM_write
...
!!**  The ASCII file elm_elems_sem2d.tab contains the list of elements concerned,
!!**  in two columns:
!!**    col 1 = element index in the DyDMG_xxx_sem2d.dat file
!!**    col 2 = element index in the global mesh
!!** 
  subroutine MAT_DyDMG_write(mw,mp,tag,ndof)
  ...
  if (.not. MAT_anyDynDamage() ) return
 ...
 ! initialize
  if (.not.initialized) then 
    iout = IO_new_unit()
    open(unit=iout,file='dyD_elems_sem2d.tab')
    k=0
    do e = 1,nelem
      if ( .not. MAT_isDynDamage(mp(e)) ) cycle
      k=k+1
      write(iout,*) k,e
      if (k==1) iol = e
    enddo
    close(iout)
    inquire( IOLENGTH=iol )  MAT_DyDMG_export(mw(iol)%DyDMG,ndof)
    !MAT_DyDMG_export(m,ndof) result(dat)
    initialized = .true.
  endif

 ! export snapshots
  write(fname,'("dyD_",A,"_sem2d.dat")') trim(tag)
  iout = IO_new_unit()
  open(unit=iout,file=trim(fname),status='replace',access='direct',recl=iol)
  k=0
  do e = 1,nelem
    if ( .not. MAT_isDynDamage(mp(e)) ) cycle
    k=k+1
    write(iout,rec=k) MAT_DyDMG_export(mw(e)%DyDMG,ndof)
  enddo
  close(iout)

  end subroutine MAT_DyDMG_write
!=======================================================================
subroutine MAT_stress_dv(s,d,v,matwrk,matpro,grid,e,ngll,ndof)
  ...
  call MAT_stress(s, MAT_strain(d,matwrk,grid,e,ngll,ndof) &
                 ,matwrk,matpro,ngll,ndof,update=.false.,tt=0.0d0)
