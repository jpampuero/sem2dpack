...
%% ************************************************************************
function [] = plot_FFA_ratio(Pcoord,data,fault,tend,save_tag,namef,opt,var)
...
for k=1:Nratio
    ...
    for j=1:li
        ...
        %resample and normalized
        mini=find([numel(s(i1).f1(:,l)),numel(s(i2).f1(:,l))] == min([numel(s(i1).f1(:,l)),numel(s(i2).f1(:,l))]),1);
        xsamp=s(ratioM(k,mini)).f1(:,l);
%         ysamp1=interp1(s(i1).f1(:,l),s(i1).fas1(:,l),xsamp);
%         ysamp2=interp1(s(i2).f1(:,l),s(i2).fas1(:,l),xsamp);
        ysamp1=interp1(s(i1).f1(:,l),s(i1).fas1(:,l)./s(i1).fas1(1,l),xsamp);
        ysamp2=interp1(s(i2).f1(:,l),s(i2).fas1(:,l)./s(i2).fas1(1,l),xsamp);

        %plot
        subplot(li,co,co*(j-1)+cc:co*(j-1)+cc-1+cf_uc);     hold on
        plot(log10(xsamp),log10(ysamp1./ysamp2),'-','color',ctab(k,:),'Linewidth',1)
...
%         k=find(l==s(i).ind);
        ...
        if k==1; text(mX+((MX-mX)*0.02),MY-((MY-mY)*0.1),['z = ',num2str(round(10*s(i).z(l)/proZ)/10),' R_0'],'FontSize', fs);end
        text(mX+((MX-mX)*0.02),MY-((MY-mY)*0.1*(k+1)),['ratio d',num2str(i1),'/d',num2str(i2)],'FontSize', fs,'color',ctab(k,:));
