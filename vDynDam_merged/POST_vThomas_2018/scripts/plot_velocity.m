...
%% ************************************************************************
function [] = plot_velocity(Pcoord,data,fault,tend,save_tag,namef,opt,var)
...
%% SELECTED SEISMOGRAMS
    ...
    %min/max
        du=(s(i).u(2:end,s(i).ind(k))-s(i).u(1:end-1,s(i).ind(k)));
        du=du/max(du);
        imX=find(du>=0.25,1);
        ...
        s(i).mY(k)=floor(2*(min(min(s(i).u(:,s(i).ind(k))))))/2;
        s(i).MY(k)=ceil(2*(max(max(s(i).u(:,s(i).ind(k))))))/2;
  end
...
%comment
disp(' ')
disp([num2str(nsS),' seismograms of ', ylab, ' for ',num2str(Nsimu), ' type of simulations '])
...
for i=1:Nsimu
    ...
    for j=1:li
        ...
        if(i>2)
            idt2=find(abs(s(2).u(:,l))==max(abs(s(2).u(:,l))));
            idti=find(abs(s(i).u(:,l))==max(abs(s(i).u(:,l))));
        ...
        plot(s(i).t+dtF,s(i).u(:,l),'color',ctab(i,:),'Linewidth',1)
...
%         k=find(l==s(i).ind);
        ...
        if cc>cf_uc, set(gca, 'yticklabel', []); else, ylabel([ylab,' (m/s)'],'Fontsize',fs),end        
        if j == 1 && cc==1
            if i==1, text(mX(k)+((MX(k)-mX(k))*0.02),mY(k)+((MY(k)-mY(k))*(0.2)),[ylab,' damage'],'FontSize', fs,'color',ctab(i,:));end
            if i==2, text(mX(k)+((MX(k)-mX(k))*0.02),mY(k)+((MY(k)-mY(k))*(0.1)),[ylab,' elasticity'],'FontSize', fs,'color',ctab(i,:));end
        end
