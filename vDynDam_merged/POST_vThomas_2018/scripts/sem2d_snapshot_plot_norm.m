...
%
function wrk = sem2d_snapshot_plot_km(field,grid,norma,vsat,wrk,target_box)
...
if ndims(field) == 3
  h=patch('Vertices',wrk.coord./norma,'Faces',wrk.indx,'FaceVertexCData',field(:),'FaceColor','interp','LineSmoothing',ANTIALIAS);
else
  h=patch('Vertices',grid.coord./norma,'Faces',wrk.indx,'FaceVertexCData',field(:),'FaceColor','interp','LineSmoothing',ANTIALIAS);
end
...
set(gca,'CLim',[min(vsat) max(vsat)])
% [min(vsat) max(vsat)]
% colorbar('vert')
