% Script to write the input file Par.inp for SEM2DPACK
...
namefile='Par.inp';
wfile = [rootD,namefoldD,'/',namefile];
fid = fopen(wfile,'wt');
...
fprintf(fid,['&MATERIAL tag=',num2str(i),', kind=''DyDMG'' / \n']);

fprintf(fid,['&MAT_DYNDAMAGE fs=',num2str(fs)]);

fprintf(fid,[', nu=',num2str(nu(j))]);
fprintf(fid,[', rho=',num2str(rho(j))]);
fprintf(fid,[', cs=',num2str(cs(j)),' \n']);

fprintf(fid,'           ');
fprintf(fid,['cp=',num2str(cp(j))]);
fprintf(fid,[', vm=',num2str(vm)]);
fprintf(fid,[', Nv=',num2str(Nv(i))]);
fprintf(fid,[', beta=',num2str(beta),' \n']);

fprintf(fid,'           ');
fprintf(fid,['omega=',num2str(omega)]);
fprintf(fid,[', KicSS=',num2str(KicSS)]);
fprintf(fid,[', a0=',num2str(a0)]);
fprintf(fid,[', fitKicD=',num2str(fitKicD),' \n']);
