...
if numel(D0)>1; D0z=sort([D0z;0]);end
...
if numel(D0)==1
    pz=[[D0z(1);pFl(2);D0z(end)],[D0z(1);pFr(2);D0z(end)]];
else
    pz=[[D0z(1);D0z(2:end-1)+pFl(2);D0z(end)],[D0z(1);D0z(2:end-1)+pFr(2);D0z(end)]];
end

%number of domain
if numel(D0)==1, numD=numel(D0)+1; else numD=numel(D0); end
% numD=numel(D0);
...
%     NELZ(i) =round(abs(D0z(i+1)-D0z(i))/h);
    NELZ(i) =round(abs(pz(i+1,1)-pz(i,1))/h);
end
...
for i=1:numD
    ...
    % tag the domain
    if numel(D0)==1,domain(i).etag(:) = 1; else domain(i).etag(:) =i; end
