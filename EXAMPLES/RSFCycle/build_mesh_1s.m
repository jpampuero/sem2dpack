...
% One-sided faults
%
% a finite single fault in the upper middle edge of the domain
%
%   | ---------------FFFFFFFFFFF----------------|
%   |                                           |
%   |                                           |
...
if FZW >= h
    ...
    e = sub2ind([NELX_R, NELZ_T],  repmat([1:NELX_R]',iFZW,1), kron(NELZ_T+1-[1: iFZW]', ones(NELX_R,1)));
    domain(3).etag(e)=tag_fz;
    
    e = sub2ind([NELX_M, NELZ_T],  repmat([1:NELX_M]',iFZW,1), kron(NELZ_T+1-[1: iFZW]', ones(NELX_M,1)));
    domain(2).etag(e)=tag_fz;
    
    e = sub2ind([NELX_L, NELZ_T],  repmat([1:NELX_L]',iFZW,1), kron(NELZ_T+1-[1: iFZW]', ones(NELX_L,1)));
...
if KVW>=h
    ...
    [indx, indz] = meshgrid([1: ikvw], NELZ_T+1-[1: ikvw]);
    e = sub2ind([NELX_R, NELZ_T],  indx(:), indz(:));
    domain(3).etag(e)=tag_kv;
        
    [indx, indz] = meshgrid([1: NELX_M], NELZ_T+1-[1: ikvw]);
    e = sub2ind([NELX_M, NELZ_T],  indx(:), indz(:));
    domain(2).etag(e)=tag_kv;
    
    [indx, indz] = meshgrid(NELX_L+1-[1 : ikvw], NELZ_T+1-[1: ikvw]);
