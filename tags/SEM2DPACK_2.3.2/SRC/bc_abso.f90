...
!
  ...
  use time_evol, only: timescheme_type, TIME_getCoefA2V
...
!     T_n = - mu/Cp * V_n - mu/Cs*(2*Cs-Cp) * U_t,t
 ...
 ! with  v_(n+alpha) = v_pre_(n+alpha) + alpha*coefA2V*a_(n+1)
 ! and coefA2V depends on the time scheme
 !
 ! => (M+alpha*coefA2V*C)*a_(n+1) = -K*d_pre_(n+alpha) -C*v_pre_(n+alpha)
 !
  M(bc%topo%node,:) = M(bc%topo%node,:)  + tim%alpha*TIME_getCoefA2V(tim)*bc%C
