...
! Isotropic or transverse anisotropic
  ...
  integer, save :: isElasticHomogeneous = 0

  public :: MAT_isElastic &
          , ISOTR_read, ANISO_read, HETE_read &
          , ELAST_init_elem_prop, ELAST_init_work &
...
!=======================================================================
! Isotropic material: RHO, VP and VS given 
...
!   coef(4) = Kmod
!
  subroutine ISOTR_read(input,iin)
  ...
  double precision :: Kmod,Kvol,young,poiss,rho,cp,cs,mu,lambda
  ...
  if(echo_input) then
    ...
    write(iout,200) cp,cs,rho,poiss,lambda,mu,Kvol,young
  endif
...
!=======================================================================
! anisotropic material: RHO,c11, c13, c33 et c44 given (Pa)
...
!   coef(4) = c33
!
  subroutine ANISO_read(input,iin)
  ...
  double precision :: rho,c11,c13,c33,c44,cp,cs

  read(iin ,*) rho, c11, c13, c33, c44

 ! a rough idea of properties
  cp  = sqrt(c11/rho)
  cs  = sqrt(c44/rho)
...
!=======================================================================
! isotropic with arbitrary distribution
!
  subroutine HETE_read(input,mode,iin)
  ...
  call MAT_setProp(input,'rho',0d0,mode,iin)
  call MAT_setProp(input,'cp',0d0,mode,iin)
  call MAT_setProp(input,'cs',0d0,mode,iin)
...
!
  subroutine ELAST_init_elem_prop(elem,ecoord)
  ...
  end subroutine ELAST_init_elem_prop
...
! ndof = 1 : SH
!      = 2 : P-SV
  subroutine ELAST_init_work(matwrk,mat_elem,grid,ndof,homo)
  ...
  if (ndof<1 .or. ndof>2) &
    call IO_abort('ELAST_init: ndof must be 1 or 2 (SH or P-SV)')
 ...
 ! test if homogeneous medium 
  homog = homo .and. MAT_isKind(mat_elem(1),isElasticHomogeneous)

 ! if box mesh and homogeneous medium: store only one element
  if (SE_isFlat(grid) .and. homog) then 
    nelem_a = 1
  else
    nelem_a = grid%nelem
  endif
  ...
  do e=1,nelem_a
    ...
    call ELAST_init_elem_work( matwrk(e)%a, grid%ngll, nelast &
  ...
  end subroutine ELAST_init_work


!---------------------------------------------------------------------
  subroutine ELAST_init_elem_work(a,ngll,nelast,xjaci,weights,mat)
  ...
  end subroutine ELAST_init_elem_work
