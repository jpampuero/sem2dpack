...
! END INPUT BLOCK
  ...
  call DIST_Read_CD(Dc,DcHet,bc%input%Dc,iin)
  call DIST_Read_CD(MuS,MuSHet,bc%input%MuS,iin)
  call DIST_Read_CD(MuD,MuDHet,bc%input%MuD,iin)
  call DIST_Read_CD(Tn,TnHet,bc%input%N,iin)
  call DIST_Read_CD(Tt,TtHet,bc%input%T,iin)
  call DIST_Read_CD(Sxx,SxxHet,bc%input%Sxx,iin)
  call DIST_Read_CD(Sxy,SxyHet,bc%input%Sxy,iin)
  call DIST_Read_CD(Sxz,SxzHet,bc%input%Sxz,iin)
  call DIST_Read_CD(Syz,SyzHet,bc%input%Syz,iin)
  call DIST_Read_CD(Szz,SzzHet,bc%input%Szz,iin)
...
! Friction parameters
  call DIST_Init_CD(bc%input%Dc,FltCoord,bc%Dc)
  call DIST_Init_CD(bc%input%MuS,FltCoord,bc%MuStatic)
  call DIST_Init_CD(bc%input%MuD,FltCoord,bc%MuDynamic)

! Initial stress
  call DIST_Init_CD(bc%input%T,FltCoord,bc%Tt0)
  call DIST_Init_CD(bc%input%N,FltCoord,bc%Tn0)
  call DIST_Init_CD(bc%input%Sxx,FltCoord,Sxx)
  call DIST_Init_CD(bc%input%Sxy,FltCoord,Sxy)
  call DIST_Init_CD(bc%input%Sxz,FltCoord,Sxz)
  call DIST_Init_CD(bc%input%Syz,FltCoord,Syz)
  call DIST_Init_CD(bc%input%Szz,FltCoord,Szz)
  nx => bc%n1(:,1)
  nz => bc%n1(:,2)
  allocate(Tx(npoin))
  allocate(Ty(npoin))
