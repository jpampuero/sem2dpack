...
subroutine solve_Newmark_alpha(pb)
  ...
  use elastic, only : ELAST_KD
  use kelvin_voigt, only : KV_etamul
 ...
 !   NOTE: f=>a, the result is actually stored in a
  if ( associated(pb%kelvin_voigt) ) then
    call ELAST_KD(pb%elast, pb%grid, d_alpha+KV_etamul(pb%kelvin_voigt,v_alpha), f)
  else
    call ELAST_KD(pb%elast, pb%grid, d_alpha, f)
  endif
...
subroutine solve_leapfrog(pb)
  ...
  if ( associated(pb%kelvin_voigt) ) then
    call ELAST_KD(pb%elast, pb%grid, d+KV_etamul(pb%kelvin_voigt,v_mid), f)
  else
    call ELAST_KD(pb%elast, pb%grid, d, f)
  endif
