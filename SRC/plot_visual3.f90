...
! routine sauvegarde fichier Visual3
  subroutine plotvisual3(field,grid,elast,it)
  ...
  !write(iout,180) 2*grid%npoin,grid%nelem*(grid%ngll-1)*(grid%ngll-1),zoffset
  write(iout,180) size(field,2)*grid%npoin &
  ...
  icell = 0
  do e=1,grid%nelem
    call ELAST_inquire(elast,e,cp=celem)
    ...
    do j=1,grid%ngll-1
      ...
      write(iout,210) icell,celem(i,j), &
  ...
  do ip=1,grid%npoin
    write(iout,205) ip,field(ip,:)/rmaxnorm
  enddo

  print *,'Max norm in output Visual3 = ',rmaxnorm
