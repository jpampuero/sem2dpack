...
! and (if requested) update plastic strain
  subroutine MAT_PLAST_stress(s,etot,m,ngll,update,E_ep,E_el)
  ...
  double precision, dimension(ngll,ngll) :: sm,tau,Y,i1,i2
  integer :: i,j
 ...
 ! test yield and update deviatoric stress
  ...
  do i=1,ngll
    if (tau(i,j)>Y(i,j)) then 
     ...
     ! Andrews (2005) corrected by Duan (2008)
      sd(i,j,:) = ( 1d0-(1d0-Y(i,j)/tau(i,j))*m%vp_factor )* sd_tr(i,j,:) 
    else
      sd(i,j,:) = sd_tr(i,j,:)
    endif
  enddo
