...
subroutine bc_DIRNEU_init(bc,tag,grid,perio)
  ...
  double precision, allocatable :: ntmp(:,:)

  call BC_inquire( grid%bounds, tag = tag, bc_topo_ptr = bc%topo )

  if (associated(bc%hstf) .or. associated(bc%vstf)) then
    allocate( bc%B(bc%topo%npoin), ntmp(bc%topo%npoin,2) )
    call BC_get_normal_and_weights(bc%topo, grid, ntmp, bc%B, &
