...
subroutine BC_DYNFLT_init(prname,DTglobal,myrank)
  ...
  if (myrank == 0) then
     if(Friction_flag == 'swf') then
        write(*,*) 'Using slip-weakening friction law.'
     elseif(Friction_flag == 'twf') then
        write(*,*) 'Using time-weakening friction law.'
     elseif(Friction_flag == 'rsf') then
        write(*,*) 'Using rate-and-state friction law.'
     endif
  endif
...
!     filename = ''
     ...
     case ('read-from-file')
        if (filename == "") then
           stop 'read-from-file option is chosen, but the filename is empty.'
        endif
...
subroutine rsf_init(f,T0,V,nucFload,coord,IIN_PAR)
  ...
  if (myrank == 0) then
     if(f%StateLaw == 1) then
        write(*,*) '    Using the aging law.'
     elseif(f%StateLaw == 2) then
        write(*,*) '    Using the slip law with strong weakening.'
     endif
  endif
