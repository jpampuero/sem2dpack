...
! SYNTAX : If the data is given in Par.inp:
!            &DIST_LINEAR n,length /
!            followed immediately by the data points, two columns per line: 
!               (1) X position, sorted in increasing order, and 
!               (2) data value at X
!          If the data is given in a separate file, same format as above:
!            &DIST_LINEAR file,length /
!
...
! END INPUT BLOCK
  ...
  NAMELIST / DIST_LINEAR / file,length,N
