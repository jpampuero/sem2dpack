...
! NOTE: same Bi on both sides, see note above
  ...
  elseif (associated(bc%rsf)) then
    call rsf_init(bc%rsf,bc%coord, time%dt )
  endif
...
! Initial friction
  ...
  call normal_init(bc%normal,time%dt,bc%T0(:,2))
...
! NOTE: file names limited to tags(1)<100
 ...
 ! adjust output timestep to the nearest multiple of dt:
  bc%oitd = max(1,nint(bc%odt/time%dt))
  bc%odt = time%dt * dble(bc%oitd)
  bc%odt = time%dt * bc%oitd
  bc%oit = nint(bc%ot1/time%dt)
 ...
 !
  ...
  NDAT = 5
  NSAMP = (time%nt -bc%oit)/bc%oitd +1
  hunit = IO_new_unit()
